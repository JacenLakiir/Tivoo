...
public class GoogleCalParserHandler extends ParserHandler{
	...
	protected class AuthorElementHandler extends ElementHandler {
		public void characters(char[] ch, int start, int length) {
			if(currentEvent != null)
				currentEvent.put("name", new String(ch, start, length));
		}
	}
	...
	protected class ContentElementHandler extends ElementHandler {
		...
		public void characters(char[] ch, int start, int length) {
			...
			if(content.startsWith("When")){
				...
				setRecurringEventTime(startCalendar, endCalendar, recurringTime, content);
				currentEvent.setStartTime(startCalendar);
				currentEvent.setEndTime(endCalendar);
	...
	public void setRecurringEventTime(Calendar startCal, Calendar endCal, String time, String eventDuration){
		...
		int endMinute = startMinute + (int)(result - (int)result);
		int endHour = 0;
		if(endMinute < 60){
